...
export function LegalNotice() {
  ...
  // キャンセルポリシーの表示テキストを生成
  const cancellationPolicyText = cancellationPolicy?.tiers?.map((tier, index) => {
    ...
    <div className="max-w-4xl mx-auto px-4 py-24 sm:px-6 lg:px-8">
      ...
      <div className="space-y-8 bg-white p-8 sm:p-12 rounded-2xl shadow-sm border border-slate-100">
        <section>
          ...
          <p className="text-sm text-slate-600">{companyInfo?.name_ja ?? "株式会社PrivateSky"}</p>
        </section>

        <section>
          <h2 className="text-sm font-bold text-slate-900 mb-2">業務責任者</h2>
          <p className="text-sm text-slate-600">{companyInfo?.representative ?? "中村 和真"}</p>
        </section>
