...
import { Clock, MapPin, Check, Camera, Plane } from "lucide-react";
...
export function Step1PlanSelection({ data, updateData, onNext }: Step1Props) {
  ...
  // ------------------------------------------------------------------
  if (!selectedPlanId) {
    const PlanGrid = ({ title, icon: Icon, plans }: { title: string, icon: any, plans: Plan[] }) => (
                ...
                >
                  ...
                  <CardContent className="p-4 flex flex-col flex-grow">
                    <div className="flex justify-between items-start mb-1">
                       <h3 className="text-sm font-bold text-slate-900 line-clamp-1 group-hover:text-vivid-blue transition-colors">{plan.title}</h3>
                    </div>
                    
                    <div className="flex items-center text-[10px] text-slate-500 mb-2 space-x-2">
                      <div className="flex items-center"><Clock className="w-3 h-3 mr-0.5" /> {plan.duration}</div>
                      <div className="flex items-center"><MapPin className="w-3 h-3 mr-0.5" /> {plan.area}</div>
                    </div>
    ...
    return (
      <div className="space-y-8">
        ...
        <div>
          <PlanGrid title="遊覧フライト" icon={Camera} plans={sightseeingPlans} />
          <PlanGrid title="移動・送迎" icon={Plane} plans={transferPlans} />
        </div>
      </div>
    );
  }
            ...
            <div className="relative h-56">
              ...
              <div className="absolute bottom-4 left-4 text-white">
                <div className="flex items-center space-x-2 text-xs mb-1">
                  <span className="bg-vivid-blue px-2 py-0.5 rounded font-bold">{selectedPlan!.area}</span>
                </div>
            ...
            <Card className="border-slate-200 shadow-sm">
              <CardContent className="p-6 space-y-6">
                ...
                <div className="bg-slate-50 p-4 rounded-xl flex items-start space-x-3 border border-slate-100">
                  ...
                  <div>
                    <span className="block font-bold text-sm text-slate-900">集合場所: 東京ヘリポート</span>
                    <span className="text-[11px] text-slate-500">東京都江東区新木場4-7-25</span>
                  </div>
                </div>
              </CardContent>
            </Card>
